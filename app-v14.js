(() => {
  ...
  function initial(){const d={};Object.keys(C.tables).forEach(t=>{d[t]={};C.shotLines.forEach(n=>{d[t][n]=blank();Object.assign(d[t][n],copy(C.defaults[t]?.[n]||{}));});});return {version:8,model:"canonical-west-v2",data:d,departurePositions:{},ui:{language:C.defaultLanguage,tableMode:C.defaultTableMode,editTable:"groot",pattern:"VIJF",direction:C.defaultDirection,departure:C.defaultDeparture,correctionMode:C.shortenedFourCushionCorrection.defaultMode,valueStyle:C.drawing.valueLabels.defaultMode,ballColor:C.drawing.balls.defaultColor,ballMarking:C.drawing.balls.defaultMarking,playRange:"lkl"}};}
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY));if([4,5,6,7,8].includes(x?.version)&&x.data){x.version=8;return x;}}catch(_){}return initial();}
  ...
  function render(){
    ...
    const pattern=C.patternByLine[ui.pattern]||(ui.language==="nl"?"Vrij spel":"Free play");$("editorTitle").textContent=`${pattern} · ${patternLabel(ui.pattern)} · ${tableLabel(ui.editTable)}`;
    const views=ui.tableMode==="beide"?["groot","klein"]:[ui.tableMode];$("drawingTitle").textContent=`${pattern} · ${patternLabel(ui.pattern)} · ${ui.direction==="west"?tr("west"):tr("east")}`;$("dragHelp").textContent=ui.language==="nl"?"Sleep de speelbal over Lijn 1; gebruik −/+ voor 1 cm.":"Drag the cue ball along Line 1; use −/+ for 1 cm.";
    renderRows();renderCorrection();dragModels={};$("svgMount").innerHTML=views.map(drawTable).join("");bindDragBalls();updateStrokeOutput();
  }
  function renderRows(){
