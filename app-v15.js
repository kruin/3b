(() => {
  ...
  function initial(){const d={};Object.keys(C.tables).forEach(t=>{d[t]={};C.shotLines.forEach(n=>{d[t][n]=blank();Object.assign(d[t][n],copy(C.defaults[t]?.[n]||{}));});});return {version:9,model:"canonical-west-v2",data:d,departurePositions:{},ui:{language:C.defaultLanguage,tableMode:C.defaultTableMode,editTable:"groot",pattern:"VIJF",direction:C.defaultDirection,departure:C.defaultDeparture,correctionMode:C.shortenedFourCushionCorrection.defaultMode,valueStyle:C.drawing.valueLabels.defaultMode,ballColor:C.drawing.balls.defaultColor,ballMarking:C.drawing.balls.defaultMarking,playRange:"lkl",lineOneMode:C.lineOneVariants.defaultMode,parallelOffset:C.lineOneVariants.parallel.defaultOffset,fanLine:C.lineOneVariants.fan.entries[0].key}};}
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY));if([4,5,6,7,8,9].includes(x?.version)&&x.data){x.version=9;return x;}}catch(_){}return initial();}
  const state=load();let ui=state.ui,dragModels={};
  function normalize(){
    ...
    Object.keys(C.tables).forEach(t=>C.shotLines.forEach(n=>C.parts.forEach(part=>{const segment=state.data[t][n][part.key],fixed=C.fixedPartBands[part.key];if(part.key!=="neus")segment.from.band=fixed.from;segment.to.band=fixed.to;["from","to"].forEach(side=>{if(Number.isFinite(Number(segment[side].value)))segment[side].value=Math.round(Number(segment[side].value));});})));
  ...
  function activeLines(){return C.parts.slice(ui.departure==="kop"?1:0);} function data(t=ui.editTable){return state.data[t][ui.pattern];}
  ...
  function render(){
    ...
    const views=ui.tableMode==="beide"?["groot","klein"]:[ui.tableMode];$("drawingTitle").textContent=`${pattern} · ${shownLine} · ${ui.direction==="west"?tr("west"):tr("east")}`;$("dragHelp").textContent=ui.language==="nl"?"Sleep de speelbal over Lijn 1; gebruik −/+ voor 1 cm.":"Drag the cue ball along Line 1; use −/+ for 1 cm.";
    renderRows();renderCorrection();renderVariantMessage();dragModels={};$("svgMount").innerHTML=views.map(drawTable).join("");bindDragBalls();updateStrokeOutput();
  }
  ...
  function renderRows(){
    ...
    lines.forEach(line=>{const s=d[line.key],row=document.createElement("tr"),name=document.createElement("td");name.className="line-name";name.innerHTML=`<span class="line-swatch" style="background:${line.color}"></span>${lineLabel(line)}`;row.append(name);
      ...
      const c=document.createElement("td"),b=document.createElement("button");b.className="row-clear";b.textContent="×";b.onclick=()=>{clear(line.key);save();render();};c.append(b);row.append(c);body.append(row);
    });updateStatus();
  }
  ...
  function valueControl(k,side,p){const td=document.createElement("td"),wrap=document.createElement("div"),minus=document.createElement("button"),value=document.createElement("button"),plus=document.createElement("button"),band=document.createElement("span");wrap.className="value-stepper";band.className="fixed-band";band.textContent=shortBand(shownBand(p.band));minus.type=value.type=plus.type="button";minus.textContent="−";plus.textContent="+";minus.className=plus.className="step-arrow";value.className="step-value";value.textContent=p.value??"—";minus.onclick=()=>changeValue(k,side,-1);plus.onclick=value.onclick=()=>changeValue(k,side,1);value.onkeydown=e=>{if(e.key==="ArrowLeft"||e.key==="ArrowDown"){e.preventDefault();changeValue(k,side,-1);}if(e.key==="ArrowRight"||e.key==="ArrowUp"){e.preventDefault();changeValue(k,side,1);}};wrap.append(band,minus,value,plus);td.append(wrap);return td;}
  function pointRange(k,side,forceLkl=false){const p=data()[k][side],fullMax=(p.band==="west"||p.band==="oost")?80:40,useLkl=(forceLkl||ui.playRange==="lkl")&&C.patternByLine[ui.pattern]==="LKL",fallback={minimum:0,maximum:fullMax};if(!useLkl)return fallback;const code=side==="from"?"V":"A",override=C.lklValueRanges.overrides?.[ui.pattern]?.[k]?.[code];if(override)return override;const base=C.defaults?.[ui.editTable]?.[ui.pattern]?.[k]?.[side]?.value;if(!Number.isFinite(Number(base)))return fallback;const offset=C.lklValueRanges.defaultOffset[code];return{minimum:Math.max(0,Math.round(Number(base)-offset)),maximum:Math.min(fullMax,Math.round(Number(base)+offset))};}
  function changeValue(k,side,delta){const p=data()[k][side],range=pointRange(k,side),step=C.lklValueRanges.integerStep,current=Number.isFinite(Number(p.value))?Math.round(Number(p.value)):range.minimum;let next=current+delta*step;if(C.lklValueRanges.wrap){if(next>range.maximum)next=range.minimum;if(next<range.minimum)next=range.maximum;}else next=Math.max(range.minimum,Math.min(range.maximum,next));p.value=next;save();render();}
  ...
  function updateStatus(){const n=segments(ui.editTable).length,total=activeLines().length,s=$("completionStatus"),isLkl=lklStatus();if(!isLkl){ui.playRange="vrij";$("playRange").value="vrij";}s.textContent=`${n}/${total} · ${isLkl?"LKL":(ui.language==="nl"?"Vrij spel":"Free play")}`;s.classList.toggle("complete",n===total);$("patternBadge").textContent=isLkl?"LKL":(ui.language==="nl"?"Vrij spel":"Free play");$("patternBadge").classList.toggle("free",!isLkl);}
  ...
  function drawTable(t){
    ...
    segs.forEach((l,index)=>{
      ...
      const fromTagPoint=seg.from.kind==="acquit"?map(stipA):GA;
  ...
  function adjustStroke(deltaCm,table=ui.editTable){const model=dragModels[table];if(!model)return;const key=positionKey(table),next=Math.max(0,Math.min(model.maxFraction,model.fraction-deltaCm/model.startDistance));state.departurePositions[key]=next;save();render();}
