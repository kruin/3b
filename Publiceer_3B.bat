...
for %%F in (index.html styles.css app-v22.js config-v22.js VERSIE.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v22.js" index.html >nul || (echo FOUT: index.html laadt app-v22.js niet.& goto :fout)
findstr /C:"config-v22.js" index.html >nul || (echo FOUT: index.html laadt config-v22.js niet.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v22.js') -and $h.Contains('config-v22.js')){exit 0};exit 1}catch{exit 1}"
