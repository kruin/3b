...
for %%F in (index.html styles.css app-v31.js config-v31.js VERSIE.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v31.js" index.html >nul || (echo FOUT: index.html laadt app-v31.js niet.& goto :fout)
findstr /C:"config-v31.js" index.html >nul || (echo FOUT: index.html laadt config-v31.js niet.& goto :fout)
findstr /C:"styles.css?v=31" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v31.js') -and $h.Contains('config-v31.js') -and $h.Contains('styles.css?v=31')){exit 0};exit 1}catch{exit 1}"
