...
for %%F in (index.html styles.css app-v44.js config-v44.js VERSIE.txt START-HIER.md .gitignore Open_3B.bat Open_KruinLines.bat Edit_Uitleg.bat) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v44.js" index.html >nul || (echo FOUT: index.html laadt app-v44.js niet.& goto :fout)
findstr /C:"config-v44.js" index.html >nul || (echo FOUT: index.html laadt config-v44.js niet.& goto :fout)
findstr /C:"styles.css?v=44" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v44.js') -and $h.Contains('config-v44.js') -and $h.Contains('styles.css?v=44')){exit 0};exit 1}catch{exit 1}"
