...
for %%F in (index.html styles.css app-v16.js config-v16.js VERSIE.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v16.js" index.html >nul || (echo FOUT: index.html laadt app-v16.js niet.& goto :fout)
findstr /C:"config-v16.js" index.html >nul || (echo FOUT: index.html laadt config-v16.js niet.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v16.js') -and $h.Contains('config-v16.js')){exit 0};exit 1}catch{exit 1}"
