...
for %%F in (index.html styles.css app-v29.js config-v29.js VERSIE.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v29.js" index.html >nul || (echo FOUT: index.html laadt app-v29.js niet.& goto :fout)
findstr /C:"config-v29.js" index.html >nul || (echo FOUT: index.html laadt config-v29.js niet.& goto :fout)
findstr /C:"styles.css?v=29" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v29.js') -and $h.Contains('config-v29.js') -and $h.Contains('styles.css?v=29')){exit 0};exit 1}catch{exit 1}"
