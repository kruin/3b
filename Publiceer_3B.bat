...
for %%F in (index.html doc\index.html styles.css app-v93.js config-v93.js VERSIE.txt START-HIER.md EDIT-WERKWIJZE.md .gitignore Kruin.html Kruin.bat Kruin.command SESSIES-INSTALLEREN.md supabase-session-schema.sql uitleg-lokaal\BEWAAR-UITLEG-HIER.txt config-input\3B-sporenbeheer-v93-menuknop-midden.xlsx) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v93.js" index.html >nul || (echo FOUT: index.html laadt app-v93.js niet.& goto :fout)
findstr /C:"config-v93.js" index.html >nul || (echo FOUT: index.html laadt config-v93.js niet.& goto :fout)
findstr /C:"styles.css?v=93" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
git check-ignore -q config-input\3B-sporenbeheer-v93-menuknop-midden.xlsx || (echo FOUT: de lokale beheersheet is niet door .gitignore beschermd.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $d=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/doc/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v93.js') -and $h.Contains('config-v93.js') -and $h.Contains('styles.css?v=93') -and $d.Contains('3B-documentatie')){exit 0};exit 1}catch{exit 1}"
