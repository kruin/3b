...
for %%F in (index.html styles.css app-v15.js config-v15.js VERSIE.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v15.js" index.html >nul || (echo FOUT: index.html laadt app-v15.js niet.& goto :fout)
findstr /C:"config-v15.js" index.html >nul || (echo FOUT: index.html laadt config-v15.js niet.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v15.js') -and $h.Contains('config-v15.js')){exit 0};exit 1}catch{exit 1}"
