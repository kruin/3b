...
for %%F in (index.html doc\index.html styles.css app-v51.js config-v51.js VERSIE.txt START-HIER.md .gitignore Kruin.html Kruin.bat Kruin.command) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"app-v51.js" index.html >nul || (echo FOUT: index.html laadt app-v51.js niet.& goto :fout)
findstr /C:"config-v51.js" index.html >nul || (echo FOUT: index.html laadt config-v51.js niet.& goto :fout)
findstr /C:"styles.css?v=51" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $d=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/doc/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v51.js') -and $h.Contains('config-v51.js') -and $h.Contains('styles.css?v=51') -and $d.Contains('3B-documentatie')){exit 0};exit 1}catch{exit 1}"
