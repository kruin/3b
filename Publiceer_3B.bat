...
for %%F in (index.html doc\index.html styles.css app-v107.js config-v107.js VERSIE.txt START-HIER.md HANDLEIDING-GEBRUIKER.md EDIT-WERKWIJZE.md .gitignore Kruin.html Kruin.bat Kruin.command SESSIES-INSTALLEREN.md supabase-session-schema.sql uitleg-lokaal\BEWAAR-UITLEG-HIER.txt) do if not exist "%%F" (echo FOUT: %%F ontbreekt.& goto :fout)
findstr /C:"Geldig voor versie 107" HANDLEIDING-GEBRUIKER.md >nul || (echo FOUT: gebruikershandleiding hoort niet bij versie 107.& goto :fout)
findstr /C:"app-v107.js" index.html >nul || (echo FOUT: index.html laadt app-v107.js niet.& goto :fout)
findstr /C:"config-v107.js" index.html >nul || (echo FOUT: index.html laadt config-v107.js niet.& goto :fout)
findstr /C:"styles.css?v=107" index.html >nul || (echo FOUT: index.html gebruikt niet de nieuwe stylesheetversie.& goto :fout)
...
powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try {$v=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/VERSIE.txt?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $h=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; $d=(Invoke-WebRequest -UseBasicParsing -Uri '%PAGES%/doc/?c=!LOCAL!^&n=!TRY!' -Headers @{'Cache-Control'='no-cache'}).Content; if($v -match 'versie %RELEASE%' -and $h.Contains('app-v107.js') -and $h.Contains('config-v107.js') -and $h.Contains('styles.css?v=107') -and $d.Contains('3B-documentatie')){exit 0};exit 1}catch{exit 1}"
